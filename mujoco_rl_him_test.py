...
def plot_simulation_results(cheater_ori_list, kf_ori_list, cheater_rpy_list, kf_rpy_list, cheater_rbody_list, kf_rbody_list):
    plt.figure(figsize=(14, 18))

    plt.subplot(3, 2, 1)
    for i in range(4):
        plt.plot([step[i] for step in cheater_ori_list], label=f"q[{i}]")
    plt.title("Cheater Orientation (Quaternion)", fontsize=10, pad=10)
    plt.legend()

    plt.subplot(3, 2, 2)
    for i in range(4):
        plt.plot([step[i] for step in kf_ori_list], label=f"q[{i}]")
    plt.title("KF Orientation (Quaternion)", fontsize=10, pad=10)
    plt.legend()

    plt.subplot(3, 2, 3)
    for i in range(3):
        plt.plot([step[i] for step in cheater_rpy_list], label=f"RPY[{i}]")
    plt.title("Cheater RPY (Roll-Pitch-Yaw)", fontsize=10, pad=10)
    plt.legend()

    plt.subplot(3, 2, 4)
    for i in range(3):
        plt.plot([step[i] for step in kf_rpy_list], label=f"RPY[{i}]")
    plt.title("KF RPY (Roll-Pitch-Yaw)", fontsize=10, pad=10)
    plt.legend()

    plt.subplot(3, 2, 5)
    for i in range(9):
        plt.plot([step.flatten()[i] for step in cheater_rbody_list], label=f"rBody[{i//3},{i%3}]")
    plt.title("Cheater rBody (Rotation Matrix)", fontsize=10, pad=10)
    plt.legend()

    plt.subplot(3, 2, 6)
    for i in range(9):
        plt.plot([step.flatten()[i] for step in kf_rbody_list], label=f"rBody[{i//3},{i%3}]")
    plt.title("KF rBody (Rotation Matrix)", fontsize=10, pad=10)
    plt.legend()
...
def run_simulation(m, d, container_cheater, container_kf, policy, config):
                ...
                for i in range(d.ncon):
                    ...
                    for gid in [g1, g2]:
                        if gid in geomid_to_leg:
                            ...
                            contact_phase[leg_idx] 

                update_state_estimators(container_cheater, container_kf, cheater_state, vectornav_data, leg_data, contact_phase)

                # Compare estimated orientation
                cheater_ori = container_cheater._data.result.orientation.copy()
                kf_ori = container_kf._data.result.orientation.copy()
                cheater_rpy = container_cheater._data.result.rpy.copy()
                kf_rpy = container_kf._data.result.rpy.copy()
                cheater_rbody = container_cheater._data.result.rBody.copy()
                kf_rbody = container_kf._data.result.rBody.copy()

                cheater_ori_list.append(cheater_ori)
                cheater_rpy_list.append(cheater_rpy)
                cheater_rbody_list.append(cheater_rbody)

                kf_ori_list.append(kf_ori)
                kf_rpy_list.append(kf_rpy)
                kf_rbody_list.append(kf_rbody)
    ...
    plot_simulation_results(cheater_ori_list, kf_ori_list, cheater_rpy_list, kf_rpy_list, cheater_rbody_list, kf_rbody_list)
